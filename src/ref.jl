    ...
    for i=2:length(ex.args)
        if ex.args[i] == sub1
            if length(ex.args)==2 && sub1type <: Array{Int}
                msg( ctx, 0, "Value at position #" * string( i-1) * " is the referenced " * string(sub1) * ". OK if it represents permutations" )
            else # almost certain it's an error
                msg( ctx, 2, "Value at position #" * string( i-1) * " is the referenced " * string(sub1) * ". Possible typo?" )
            end
        end
