    ...
    if ctx.quoteLvl == 0
        for v in unused
            if !pragmaexists("Ignore unused " * utf8(v), ctx) && v != :_
                ...
                msg(ctx, :W341, v, "Local vars declared but not used: " * utf8(v))
            end
        end
        if checkargs
            ...
            for v in unusedargs
                if !pragmaexists("Ignore unused " * utf8(v), ctx) && v != :_
                    ...
                    msg(ctx, :I382, v, "argument declared but not used: " * utf8(v))
                end
            end
        end
    end
...
function registersymboluse(sym::Symbol, ctx::LintContext, strict::Bool=true)
    ...
    if ctx.quoteLvl == 0
        msg(ctx, :E321, sym, "use of undeclared symbol " * utf8(sym))
    elseif ctx.isstaged
        msg(ctx, :I371, sym, "use of undeclared symbol " * utf8(sym))
    end
    return :Any
end
...
function lintglobal(ex::Expr, ctx::LintContext)
    for sym in ex.args
        ...
        elseif isexpr(sym, ASSIGN_OPS)
            ...
            msg(ctx, :E134, sym, "unknown global pattern $sym")
        end
    end
end
...
function lintlocal(ex::Expr, ctx::LintContext)
    ...
    for sube in ex.args
        ...
        if typeof(sube) != Expr
            msg(ctx, :E135, sube, "local declaration $sube not understood by Lint")
            continue
        end
        if sube.head == :(=)
...
function resolveLHSsymbol(ex, syms::Array{Any,1}, ctx::LintContext, typeassert::Dict{Symbol,Any})
    ...
    elseif typeof(ex) == Expr
        ...
        elseif ex.head == :(.) ||   # a.b = something
            ...
            msg(ctx, :I171, ex, "LHS in assignment not understood by Lint. please check: $ex")
        end
    else
        msg(ctx, :I171, ex, "LHS in assignment not understood by Lint. please check: $ex")
    end
...
function lintassignment(ex::Expr, assign_ops::Symbol, ctx::LintContext; islocal = false, isConst=false, isGlobal=false, isForLoop=false) # is it a local decl & assignment?
    ...
    if isForLoop
        ...
        if VERSION < v"0.4.0-dev+4139"
            if rhstype <: Tuple && length(rhstype) != tuplelen
                msg(ctx, :I474, rhstype, "iteration generates tuples of $rhstype. " *
    ...
    if VERSION < v"0.4.0-dev+4139"
        if typeof(rhstype) != Symbol && rhstype <: Tuple && length(rhstype) != tuplelen && !isForLoop
            if length(syms) > 1
                msg(ctx, :E418, rhstype, "RHS is a tuple of $rhstype. N of " *
    ...
    for (symidx, s) in enumerate(syms)
        if typeof(s) != Symbol # a.b or a[b]
            if isexpr(s, [:(.), :ref])
                ...
                if containertype != Any && typeof(containertype) == DataType && !containertype.mutable
                    msg(ctx, :E525, s.args[1], "$(s.args[1]) is of an immutable type $(containertype)")
                end
            end
        ...
        if string(s) == ctx.scope && !islocal
            msg(ctx, :W355, ctx.scope, "variable $(ctx.scope) == function name")
        end
        if s == :call
            msg(ctx, :E332, s, "$(s) should not be used as a variable name")
        elseif in(s, knownsyms)
            if in(s, [:e, :pi, :eu, :catalan, :eulergamma, :golden, :π, :γ, :φ])
                if ctx.file != "constants.jl"
                    msg(ctx, :W351, s, "you are redefining a mathematical constant $(s)")
                end
            else
                msg(ctx, :W356, s, "$s as a local variable might cause " *
        ...
        try
            if haskey(typeassert, s)
                ...
                if typeof(dt) == DataType
                    ...
                    if !isAnyOrTupleAny(dt) && !isAnyOrTupleAny(rhstype) && !(rhstype <: dt)
                        msg(ctx, :I572, "assert $(s) type= $(dt) but assign a " *
                            "value of $(rhstype)")
                    end
                else
            ...
            msg(ctx, :W251, s, "$(er); $(ex); Symbol=$(s); rhstype=$(rhst)")
            if haskey(typeassert, s)
                vi.typeexpr = typeassert[s]
            end
        end
        ...
        if islocal
            ...
            for i in length(ctx.callstack[end].localvars):-1:1
                if haskey(ctx.callstack[end].localvars[i], s)
                    ...
                    elseif !isAnyOrTupleAny(vi.typeactual) && typeof(vi.typeactual) != Symbol && !(vi.typeactual <: prevvi.typeactual) &&
                        ...
                        msg(ctx, :W545, s, "previously used $(s) has apparent type " *
            ...
            if !found && in(s, ctx.callstack[end].oosvars)
                msg(ctx, :I482, s, "$(s) has been used in a local scope. Improve " *
            ...
            if !found && !isGlobal && !haskey(ctx.callstack[end].declglobs, s)
                for i in length(ctx.callstack)-1:-1:1
                    if haskey(ctx.callstack[i].declglobs, s) &&
                        ...
                        splice!(l, 1)
                        src = join(l, "\n")
                        msg(ctx, :I391, s, "$(s) is also a global, from :  $(src); Please check")
                        break;
                    end
                end
            end
